      ...
      <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
          <a href={`https://yandex.ru/search/?text=${encodeURIComponent(movie.titleRu + ' фильм ' + movie.year)}`}
            ...
            style={{ padding: '10px 16px', borderRadius: 10, border: '1px solid hsl(220 15% 18%)', color: '#787e8a', fontSize: 13, fontWeight: 500, textDecoration: 'none', flexShrink: 0 }}>
            Яндекс
          </a>
          <button onClick={() => { onRate(movie); }}
            style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid hsl(220 15% 18%)', background: 'none', color: '#ece9e0', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s' }}
            onMouseDown={e => e.currentTarget.style.transform = 'scale(0.96)'}
            onMouseUp={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.background = 'hsl(38 90% 55%)'; e.currentTarget.style.color = '#0d1117'; e.currentTarget.style.borderColor = 'hsl(38 90% 55%)'; }}
