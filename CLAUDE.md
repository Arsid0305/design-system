...
Claude инициирует проверку сам перед первьм деплоем в `main`. Молча не пропускать.
...
Схема: `ветка claude/...` → `dev` (авто) → `main` (авто)

1. Claude пишет код → пушит в ветку `claude/...`
2. `automerge.yml` автоматически мержит ветку в `dev` (PR не нужен)
3. `promote.yml` автоматически мержит `dev` → `main` после каждого пуша в `dev`
